    ...
    def multi_solve_environment(self):
        ...
        for arch_epoch in range(self.arch_epochs):
            ...
            for episode in range(self.episodes):
                actions_p, actions_log_p, actions_index, scope_dict = self.controller.sample()
                ...
                if episode < self.episodes // 3:
                    worker = Worker(actions_p, actions_log_p, actions_index, scope_dict, self.args, 'cuda:0')
                elif self.episodes // 3 <= episode < 2 * self.episodes // 3:
                    worker = Worker(actions_p, actions_log_p, actions_index, scope_dict, self.args, 'cuda:1')
                else:
                    worker = Worker(actions_p, actions_log_p, actions_index, scope_dict, self.args, 'cuda:3')
